...
export default authMiddleware(async (req: AuthenticatedRequest, res: NextApiResponse) => {
  try {
    if (req.method === 'GET') {
      const result = await executeCommand('docker_network_ls');
      const lines = result.stdout.trim().split('\n').filter(Boolean);
      const networks = lines.map(l => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
    ...
    if (req.method === 'POST') {
      ...
      await executeCommand('docker_network_create', [name]);
      return res.status(200).json({ success: true, data: { created: true, name } });
    }
    if (req.method === 'DELETE') {
      const { name } = req.body;
      if (!name) return res.status(400).json({ success: false, error: 'Nome é obrigatório' });
      await executeCommand('docker_network_rm', [name]);
